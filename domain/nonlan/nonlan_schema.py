...
class Nonlan(BaseModel):        # BaseModel : 입력된 데이터가 모델의 기대하는 형식과 일치하는지 확인할 수 있다.
...
class NonlanList(BaseModel):
    total: int = 0
    nonlan_list: list[Nonlan] = []


class NonlanDelete(BaseModel):
    nonlan_id: int


class NonlanUpdate(NonlanCreate):
    nonlan_id: int
